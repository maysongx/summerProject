//文章
var pagingHtmlObj1 = $(".article .paging");
var pagingSet1 = {
    curObj: 1,
    curPage: 1,
    totalPage: 0,
...
(function ($) {
    $.paging = {
        init: function () {
            ...
            //初始化分页点击事件
            $.paging.initPage(pagingHtmlObj1, pagingSet1);
                ...
                if (data.success && data) {
                    if (data.data.aaData.length > 0) {
                        ...
                        var htmls = '<div class="norecord">' +
                            '<span>你还没收藏过视频哦^_^</span>' +
                            '<a href="' + $.public.MOBILEURL + '/showback/" target="_blank">看看有什么视频</a>' +
                            '</div>';
                        $(".article ").html(htmls);
                    }
                } else {
        ...
        "initPage": function ($dom, pagingSet) {
            //加载更多
            $dom.find('.loadMore').click(function () {
